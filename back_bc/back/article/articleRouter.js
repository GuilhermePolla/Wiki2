...
articleRouter.post("/save", authentication, async (req, res) => {
  try {
    const newArticle = await articleController.saveArticle(
      req.body,
      req.author
    );
    ...
    res
      .status(500)
      .json({ erro: "Erro ao criar o artigo", detalhes: error.message });
  }
});
...
articleRouter.post("/edit/:id", async (req, res) => {
  try {
    ...
    if (!article) {
      res.status(404).json({ erro: `Artigo id ${id} não encontrado` });
      return;
    }
